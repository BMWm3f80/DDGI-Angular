...
export class GridService {
  ...
  geDtOptions(code: string) {
    this.http.post<any>(`${environment.apiUrl}/api/grid/`, {gridCodeName: code}).subscribe({
        next: response => {
          this.myDt = response;
  ...
  initDt(code: string): Observable<any>{
    return this.http.post<any>(`${environment.apiUrl}/api/grid/`, {gridCodeName: code});
  }

  dtObjToHttpParam(obj: any) {
  ...
  applyFilterParams(params: HttpParams, filterParams: any){
